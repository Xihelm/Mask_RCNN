...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks,
    ...
    masks = np.zeros(
        (config.TRAIN_ROIS_PER_IMAGE, config.MASK_SHAPE[0],
    ...
    for i in pos_ids:
        ...
        mask = skimage.transform.resize(
            m, config.MASK_SHAPE, order=1, mode="constant", anti_aliasing=True)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if config.GPU_COUNT > 1:
            from ai_plaftorm.extenral.mrcnn.parallel_model import ParallelModel
